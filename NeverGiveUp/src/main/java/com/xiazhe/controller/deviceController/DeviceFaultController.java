...
public class DeviceFaultController {
    ...
    public QueryJsonBean<DeviceFault> queryEmployee(String searchValue, Integer page, Integer rows, HttpServletRequest request) {
        ...
        if (request.getRequestURI().contains("deviceFaultId"))
            deviceFaultList = deviceFaultService.queryDeviceFaultsByFaultId(searchValue);
        if (request.getRequestURI().contains("deviceName"))
            deviceFaultList = deviceFaultService.queryDeviceFaultsByDeviceName(searchValue);
        QueryJsonBean<DeviceFault> departmentQueryJsonBean = new QueryJsonBean<>();
        departmentQueryJsonBean.setTotal((int) resultPage.getTotal());
        departmentQueryJsonBean.setRows(deviceFaultList);
        return departmentQueryJsonBean;
    ...
    @ResponseBody
    public DeviceFault restType(@PathVariable String id) {
        return deviceFaultService.selectOneDeviceFaultById(id);
    }
